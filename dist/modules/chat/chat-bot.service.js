...
const appointment_type_repository_1 = require("../../modules/appointment/repositories/appointment-type.repository");
const health_professional_repository_1 = require("../../modules/appointment/repositories/health-professional.repository");
const appointment_request_service_1 = require("../../modules/appointment/services/appointment-request.service");
const availability_service_1 = require("../../modules/appointment/services/availability.service");
...
class ChatBotService {
    ...
    async startNewSession() {
        ...
        const session = await this.chatSessionRepository.create({
            ...
            current_step: chat_session_model_1.ChatStepType.VALIDATE_DOCUMENT,
            status: chat_session_model_1.ChatSessionStatus.ACTIVE,
            chat_data: {},
            last_interaction_at: new Date(),
        });
        // Send welcome message
        await this.sendBotMessage(sessionId, '👋 !Hola, me llamo Eli! ¡Bienvenido al sistema de agendamiento de citas médicas! Por favor, ingresa tu número de documento (cédula) para continuar:');
        return session;
    }
    /**
    ...
    async getOrCreateSession(sessionId) {
        if (sessionId) {
            ...
            if (existingSession &&
                existingSession.status === chat_session_model_1.ChatSessionStatus.ACTIVE) {
                // Update last interaction time
                await this.chatSessionRepository.update(existingSession.id, { last_interaction_at: new Date() }, 'ChatSession');
    ...
    async processMessage(sessionId, message) {
        ...
        if (!session || session.status !== chat_session_model_1.ChatSessionStatus.ACTIVE) {
            throw new error_handler_1.BadRequestError('Sesión no válida o inactiva');
        }
        // Record incoming message
        await this.chatMessageRepository.create({
            session_id: sessionId,
            direction: chat_message_model_1.MessageDirection.INCOMING,
            message_content: message,
        });
        // Update last interaction time
        await this.chatSessionRepository.update(session.id, { last_interaction_at: new Date() }, 'ChatSession');
        // Process based on current step
        try {
            switch (session.current_step) {
                case chat_session_model_1.ChatStepType.VALIDATE_DOCUMENT:
                    await this.processDocumentValidation(session, message);
                    break;
                case chat_session_model_1.ChatStepType.SELECT_CITY:
                    await this.processCitySelection(session, message);
                    break;
                case chat_session_model_1.ChatStepType.SELECT_SPECIALTY:
                    await this.processSpecialtySelection(session, message);
                    break;
                case chat_session_model_1.ChatStepType.SELECT_APPOINTMENT_TYPE:
                ...
                //   break;
                case chat_session_model_1.ChatStepType.CONFIRM_APPOINTMENT:
            ...
            await this.sendBotMessage(sessionId, 'Hubo un error al procesar tu solicitud. Por favor, intenta nuevamente.');
        }
    }
    /**
     * Process document validation step
     */
    async processDocumentValidation(session, message) {
        // Clean and validate document input
        const document = message.trim().replace(/\D/g, '');
        if (document.length < 6 ||
            document.length > 10 ||
            !/^\d+$/.test(document)) {
            await this.sendBotMessage(session.session_id, 'El número de documento debe tener entre 6 y 10 dígitos numéricos. Por favor, inténtalo nuevamente:');
            return;
        }
        ...
        try {
            ...
            if (!patient) {
                await this.sendBotMessage(session.session_id, 'No hemos encontrado un paciente registrado con ese número de documento. Por favor, verifica el número o contacta con soporte.');
                return;
            }
            ...
            await this.chatSessionRepository.update(session.id, {
                ...
                chat_data: chatData,
                current_step: chat_session_model_1.ChatStepType.SELECT_CITY,
            }, 'ChatSession');
            // Send confirmation and move to city selection
            await this.sendBotMessage(session.session_id, `¡Hola ${patient.nombre}! Por favor, selecciona la ciudad donde deseas agendar tu cita:`);
            ...
            logger_1.default.error(`Error validating document: ${error}`);
            await this.sendBotMessage(session.session_id, 'Hubo un problema al validar tu documento. Por favor, inténtalo nuevamente:');
        }
    }
    /**
     * Process city selection step
     */
    async processCitySelection(session, message) {
        const cities = await this.getAvailableCities();
        ...
        if (/^\d+$/.test(cleanMessage)) {
            ...
            selectedCity =
                cities.find((city) => city.toLowerCase() === cleanMessage ||
                    city.toLowerCase().includes(cleanMessage)) || null;
        }
        if (!selectedCity) {
            // Send error and list cities again
            await this.sendBotMessage(session.session_id, 'No hemos reconocido esa ciudad. Por favor, selecciona una de las siguientes opciones:');
            const cityMessage = cities
                .map((city, index) => `${index + 1}. ${city}`)
                .join('\n');
            await this.sendBotMessage(session.session_id, cityMessage);
        ...
        await this.chatSessionRepository.update(session.id, {
            chat_data: chatData,
            current_step: chat_session_model_1.ChatStepType.SELECT_SPECIALTY,
        }, 'ChatSession');
        // Send confirmation and move to specialty selection
        await this.sendBotMessage(session.session_id, `Has seleccionado ${selectedCity}. Ahora, elige la especialidad médica para tu cita:`);
     ...
     */
    async processSpecialtySelection(session, message) {
        const chatData = session.chat_data || {};
        const city = chatData.selectedCity;
        if (!city) {
            // Something went wrong, go back to city selection
            await this.chatSessionRepository.update(session.id, { current_step: chat_session_model_1.ChatStepType.SELECT_CITY }, 'ChatSession');
            await this.sendBotMessage(session.session_id, 'Hubo un problema con la selección de ciudad. Por favor, selecciona nuevamente la ciudad:');
            const cities = await this.getAvailableCities();
            const cityMessage = cities
                .map((city, index) => `${index + 1}. ${city}`)
                .join('\n');
        ...
        if (/^\d+$/.test(cleanMessage)) {
            ...
            // Try to match by name
            selectedSpecialty =
                specialties.find((specialty) => specialty.toLowerCase() === cleanMessage ||
                    specialty.toLowerCase().includes(cleanMessage)) || null;
        }
        if (!selectedSpecialty) {
            // Send error and list specialties again
            await this.sendBotMessage(session.session_id, 'No hemos reconocido esa especialidad. Por favor, selecciona una de las siguientes opciones:');
            const specialtyMessage = specialties
                .map((specialty, index) => `${index + 1}. ${specialty}`)
                .join('\n');
            await this.sendBotMessage(session.session_id, specialtyMessage);
        ...
        if (selectedSpecialty.toLowerCase() === 'medicina general') {
            await this.chatSessionRepository.update(session.id, {
                chat_data: chatData,
                current_step: chat_session_model_1.ChatStepType.SELECT_APPOINTMENT_TYPE,
            }, 'ChatSession');
            // Enviar mensaje para seleccionar tipo de cita
            await this.sendBotMessage(session.session_id, `Has seleccionado ${selectedSpecialty}. ¿Qué tipo de cita prefieres?
  ...
  Por favor, selecciona una opción (1 o 2):`);
            ...
            await this.chatSessionRepository.update(session.id, {
                chat_data: chatData,
                current_step: chat_session_model_1.ChatStepType.CONFIRM_APPOINTMENT,
            }, 'ChatSession');
  ...
  ¿Deseas confirmar esta solicitud de cita? (Responde SI para confirmar o NO para cancelar)`;
    ...
    async processAppointmentTypeSelection(session, message) {
        const chatData = session.chat_data || {};
        const cleanMessage = message.trim().toLowerCase();
        ...
        if (!appointmentType) {
            ...
            await this.sendBotMessage(session.session_id, 'No hemos reconocido esa opción. Por favor, selecciona el tipo de cita: 1. Virtual 2. Presencial');
            return;
        }
  ...
  ¿Deseas confirmar esta solicitud de cita? (Responde SI para confirmar o NO para cancelar)`;
     ...
     */
    async processAppointmentConfirmation(session, message) {
        const cleanMessage = message.trim().toLowerCase();
        ...
        if (cleanMessage === 'si' ||
            ...
            `,
                    ...
                    if (result.success) {
                        logger_1.default.info(`Email sent successfully for appointment ID: ${appointmentData}`);
                    }
                    else {
                        logger_1.default.error(`Error sending email for appointment: ${result.error}`);
                    }
                }
                catch (error) {
                    logger_1.default.error(`Error al enviar notificación de la cita:`, error);
                ...
                await this.chatSessionRepository.update(session.id, {
                    ...
                    current_step: chat_session_model_1.ChatStepType.COMPLETED,
                    status: chat_session_model_1.ChatSessionStatus.COMPLETED,
                }, 'ChatSession');
                ...
                await this.sendBotMessage(session.session_id, `✅ ¡Tu solicitud de cita ha sido registrada exitosamente!
  ...
  ¿Necesitas algo más? Puedes iniciar una nueva conversación para solicitar otra cita.`);
                ...
                await this.sendBotMessage(session.session_id, 'Lo sentimos, hubo un problema al crear tu cita. Por favor, inténtalo nuevamente más tarde o contacta con soporte.');
            }
        }
            ...
            // Send message
            await this.sendBotMessage(session.session_id, 'Has cancelado la cita. ¿Deseas iniciar nuevamente el proceso? Por favor, selecciona la ciudad donde deseas agendar tu cita:');
            // Get available cities and send as options
            const cities = await this.getAvailableCities();
            const cityMessage = cities
                .map((city, index) => `${index + 1}. ${city}`)
            ...
            // Invalid response
            await this.sendBotMessage(session.session_id, 'Por favor, responde SI para confirmar la cita o NO para cancelar:');
        }
    }
    /**
     * Helper methods
     ...
     */
    async sendBotMessage(sessionId, message) {
        await this.chatMessageRepository.create({
            session_id: sessionId,
            direction: chat_message_model_1.MessageDirection.OUTGOING,
            message_content: message,
        });
    }
    /**
   ...
   */
     ...
     */
    async getAvailableProfessionals(city, specialty) {
        try {
            ...
            // Filter by specialty
            return professionals.filter((p) => p.specialty.toLowerCase() === specialty.toLowerCase());
        }
        catch (error) {
            logger_1.default.error(`Error getting professionals: ${error}`);
            return [];
        }
    }
}
