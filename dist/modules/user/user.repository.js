...
class UserRepository extends base_repository_1.BaseRepository {
     ...
     * @returns El usuario encontrado o null
     */
    async findByIdentification(numberid, includePassword = false) {
        const options = {
            where: { numberid }
        };
        if (includePassword) {
    ...
    async findByFilters(filters) {
        ...
        if (filters.email) {
            whereOptions.email = (0, typeorm_1.ILike)(`%${filters.email}%`);
        }
        if (filters.name) {
            whereOptions.name = (0, typeorm_1.ILike)(`%${filters.name}%`);
        }
        if (filters.lastname) {
            whereOptions.lastname = (0, typeorm_1.ILike)(`%${filters.lastname}%`);
        }
        if (filters.typeid) {
            whereOptions.typeid = filters.typeid;
        }
        if (filters.numberid) {
            whereOptions.numberid = (0, typeorm_1.ILike)(`%${filters.numberid}%`);
        }
