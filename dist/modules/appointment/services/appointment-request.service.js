...
class AppointmentRequestService {
     ...
     */
    async requestAppointment(data, userId) {
        // 1. Validar la entrada de datos
        if (!data.patient_id || !data.date || !data.time) {
            throw new error_handler_1.BadRequestError('Faltan datos requeridos para la solicitud de cita');
        }
        // 2. Convertir fecha y hora a objetos Date
        const [year, month, day] = data.date.split('-').map(Number);
        const [hour, minute] = data.time.split(':').map(Number);
        if (isNaN(year) || isNaN(month) || isNaN(day) || isNaN(hour) || isNaN(minute)) {
            throw new error_handler_1.BadRequestError('Formato de fecha u hora inválido');
        }
        ...
        try {
            ...
            await this.notificationService.createNotification({
                ...
                type: notification_model_1.NotificationType.APPOINTMENT_CREATED,
    ...
    async checkAvailability(professionalId, startTime, endTime) {
        ...
        return availableSlots.some(slot => startTime >= slot.start &&
            endTime <= slot.end);
    }
}
