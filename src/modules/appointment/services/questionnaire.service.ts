...
import { QuestionnaireRepository, QuestionnaireQuestionRepository, QuestionnaireResponseRepository, QuestionResponseRepository } from '../repositories/questionnaire.repository';
...
export class QuestionnaireService {
  ...
  private questionRepository: QuestionnaireQuestionRepository;
  private responseRepository: QuestionnaireResponseRepository;
  ...
  constructor() {
    ...
    this.questionRepository = new QuestionnaireQuestionRepository();
    this.responseRepository = new QuestionnaireResponseRepository();
  ...
  async getQuestionnaireById(id: number): Promise<Questionnaire> {
    ...
    if (!questionnaire) {
      throw new NotFoundError(`Cuestionario con ID ${id} no encontrado`);
    }
   ...
   */
  async getQuestionnairesByAppointmentType(appointmentTypeId: number): Promise<Questionnaire[]> {
  ...
  async createQuestionnaire(data: Partial<Questionnaire>): Promise<Questionnaire> {
    ...
    if (!data.title) {
      throw new BadRequestError('El título del cuestionario es requerido');
    }
   ...
   */
  async deactivateQuestionnaire(id: number): Promise<{ success: boolean; message: string }> {
    // Verificar que el cuestionario existe
    await this.getQuestionnaireById(id);
   ...
   */
  async addQuestion(questionnaireId: number, data: Partial<QuestionnaireQuestion>): Promise<QuestionnaireQuestion> {
    // Verificar que el cuestionario existe
    ...
    if (!data.question_text || !data.question_type) {
      throw new BadRequestError('El texto y tipo de pregunta son requeridos');
    }
   ...
   */
  async updateQuestion(id: number, data: Partial<QuestionnaireQuestion>): Promise<QuestionnaireQuestion> {
    ...
    if (data.questionnaire_id && data.questionnaire_id !== question.questionnaire_id) {
      throw new BadRequestError('No se puede cambiar el cuestionario al que pertenece la pregunta');
    }
   ...
   */
  async reorderQuestions(questionnaireId: number, questionIds: number[]): Promise<{ success: boolean; message: string }> {
    // Verificar que el cuestionario existe
    await this.getQuestionnaireById(questionnaireId);
    ...
    for (const id of questionIds) {
      if (!questionMap.has(id)) {
        throw new BadRequestError(`La pregunta con ID ${id} no pertenece a este cuestionario`);
      }
    }
   ...
   */
  async getQuestionnaireForAppointment(appointmentId: number): Promise<Questionnaire | null> {
    // Obtener la cita
    ...
    if (!questionnaires || questionnaires.length === 0) {
      const generalQuestionnaires = await this.questionnaireRepository.findAll({
        where: { appointment_type_id: IsNull(), is_active: true },
        relations: ['questions']
      });
  ...
  async saveQuestionnaireResponse(
    ...
    patientId: number,
    responses: Array<{ question_id: number; response_text?: string; response_data?: any }>
    ...
    for (const response of responses) {
      if (!questionIds.includes(response.question_id)) {
        throw new BadRequestError(`La pregunta con ID ${response.question_id} no pertenece a este cuestionario`);
      }
    }
   ...
   */
  async getQuestionnaireResponseForAppointment(appointmentId: number): Promise<QuestionnaireResponse | null> {
   ...
   */
  async getQuestionnaireResponsesByPatient(patientId: number): Promise<QuestionnaireResponse[]> {
