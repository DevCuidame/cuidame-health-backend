...
export class ContactService {
   ...
   */
  async getContactsByUserId(userId: number): Promise<Contact | null> {
   ...
   */
  async createOrUpdateContact(userId: number, contactData: CreateContactDto): Promise<Contact> {
    // Validar que al menos un contacto tenga nombre y teléfono
    const hasValidContact = (
      (contactData.nombre1 && contactData.telefono1) ||
      (contactData.nombre2 && contactData.telefono2) ||
      (contactData.nombre3 && contactData.telefono3)
    );

    if (!hasValidContact) {
      throw new BadRequestError('Debe proporcionar al menos un contacto con nombre y teléfono');
    }
    ...
    if ((contactData.nombre1 && !contactData.telefono1) || (!contactData.nombre1 && contactData.telefono1)) {
      throw new BadRequestError('El primer contacto debe tener tanto nombre como teléfono');
    }

    if ((contactData.nombre2 && !contactData.telefono2) || (!contactData.nombre2 && contactData.telefono2)) {
      throw new BadRequestError('El segundo contacto debe tener tanto nombre como teléfono');
    }

    if ((contactData.nombre3 && !contactData.telefono3) || (!contactData.nombre3 && contactData.telefono3)) {
      throw new BadRequestError('El tercer contacto debe tener tanto nombre como teléfono');
    }
   ...
   */
  async deleteSpecificContact(userId: number, contactNumber: number): Promise<Contact> {
    // Buscar contactos del usuario
    const contacts = await this.contactRepository.findByUserId(userId);
    
    if (!contacts) {
      throw new NotFoundError('No se encontraron contactos para este usuario');
    }
    ...
    const contactsRemaining = 
      (contactNumber !== 1 && contacts.nombre1 && contacts.telefono1) ||
    ...
    if (!contactsRemaining) {
      throw new BadRequestError('Debe mantener al menos un contacto');
    }
